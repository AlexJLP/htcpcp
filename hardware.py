...
class HardwareController:
    ...
    def _load_config(self):
        try:
            with open("recipes.yaml", "r") as f:
                ...
                self.calibration = config.get("calibration", {"ml_per_second": 5.0, "steps_per_gram": 100})
    ...
    def _setup_hardware(self):
        ...
        try:
            ...
            self.distance_sensor = DistanceSensor(echo=self.echo_pin, trigger=self.trig_pin, max_distance=0.5)
    ...
    def read_temp(self) -> float:
        ...
        if not self.temp_file: return 0.0
        try:
            valid_readings = []
            for _ in range(3):
                ...
                if lines and "YES" in lines[0]:
                    ...
                    if equals_pos != -1:
                        ...
                        if 0.0 <= val <= 100.0: valid_readings.append(val)
                import time
                time.sleep(0.1)
            return sum(valid_readings) / len(valid_readings) if valid_readings else self.pot.temperature
        except Exception as e:
    ...
    def _update_display(self):
        if self.use_mock or not self.oled: return
        ...
        with canvas(self.oled) as draw:
            ...
            if self.pot.status not in [PotStatus.IDLE, PotStatus.READY, PotStatus.NO_MUG]:
    ...
    async def run_brew_sequence(self, recipe_name: str = "default"):
        ...
        log.info("hardware.brew_recipe_resolved", name=recipe.get("name"), target_temp=recipe.get("target_temp"))
        self.pot.current_phase = -1
        self.pot.progress = 0.0
        log.info("hardware.brew_sequence_start", pot_id=self.pot.id, recipe=recipe['name'])
        
        # 1. Grounds
        self.pot.status = PotStatus.DISPENSING_GROUNDS
        steps = int(recipe.get("coffee_grams", 15) * self.calibration.get("steps_per_gram", 100))
        log.info("hardware.dispensing_grounds", steps=steps)
        if self.dir_pin: self.dir_pin.off() 
        ...
        log.info("hardware.heating_start", current_temp=self.pot.temperature, target=target_temp)
        while self.pot.temperature < target_temp:
            if not self.pot.mug_present: # Emergency stop
                ...
                self.pot.status = PotStatus.NO_MUG
                return
            log.debug("hardware.heating_progress", current=self.pot.temperature, target=target_temp)
        ...
        total_pour_time = sum((p.get("water_ml", 0) / ml_per_sec) + p.get("pause_seconds", 0) for p in phases)
        elapsed_time = 0.0
        ...
        for i, phase in enumerate(phases):
            ...
            log.info("hardware.pump_start", action=action, ml=phase.get("water_ml"), duration=round(duration, 2))
            if self.pump_pin: self.pump_pin.on()
            ...
            if self.pump_pin: self.pump_pin.off()
    ...
    def stop_all(self):
        if self.pump_pin: self.pump_pin.off()
        if self.step_pin: self.step_pin.off()

    def _run_stepper(self, steps: int):
        import time
        if self.use_mock: time.sleep(steps * 0.002); return
        for _ in range(steps):
            self.step_pin.on(); time.sleep(0.002)
            self.step_pin.off(); time.sleep(0.002)
